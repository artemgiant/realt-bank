...
@php
    // Получаем данные локации из комплекса
    $cityData = $complex->city;
    $districtData = $complex->district;
    $zoneData = $complex->zone;
    $stateData = $cityData?->state;
    $countryData = $stateData?->country;

    // Получаем улицу из первого блока, если есть
    $firstBlock = $complex->blocks->first();
    $streetData = $firstBlock?->street;
@endphp
...
<div class="item selects w16">
    ...
    <div class="state-search-wrapper">
        <input type="text"
               ...
               autocomplete="off"
               value="{{ old('state_name', $stateData?->name ?? '') }}"
               required>
        ...
        {{-- Кнопка очистки --}}
        <button type="button" class="state-search-clear" @if(!$stateData) style="display: none;" @endif>
        ...
        <input type="hidden" name="state_id" id="state_id" value="{{ old('state_id', $stateData?->id) }}" required>
        <input type="hidden" name="state_name" id="state_name" value="{{ old('state_name', $stateData?->name) }}">
        <input type="hidden" name="country_id" id="country_id" value="{{ old('country_id', $countryData?->id) }}">
        <input type="hidden" name="country_name" id="country_name" value="{{ old('country_name', $countryData?->name) }}">
    </div>
</div>
...
<div class="item w33">
    ...
    <div class="location-search-wrapper">
        <input type="text"
               ...
               autocomplete="off"
               value="{{ old('street_name', $streetData?->name ?? '') }}"
               required>
        ...
        {{-- Кнопка очистки --}}
        <button type="button" class="location-search-clear" @if(!$streetData) style="display: none;" @endif>
            <svg width="10" height="10" viewBox="0 0 10 10" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M0.878207 9.87891C0.653517 9.87891 0.428717 9.79318 0.257263 9.62173C-0.0857544 9.27871 -0.0857544 8.72285 0.257263 8.37984L8.37992 0.257181C8.72294 -0.085727 9.27879 -0.085727 9.62181 0.257181C9.96472 0.600089 9.96472 1.15605 9.62181 1.49896L1.49915 9.62162C1.3277 9.79318 1.1029 9.87891 0.878207 9.87891Z" fill="currentColor"/>
                <path d="M9.00086 9.8788C8.77606 9.8788 8.55137 9.79307 8.37992 9.62162L0.257263 1.49896C-0.0857544 1.15605 -0.0857544 0.600089 0.257263 0.257181C0.600171 -0.085727 1.15613 -0.085727 1.49904 0.257181L9.6217 8.37984C9.96461 8.72285 9.96461 9.27871 9.6217 9.62173C9.45035 9.79307 9.22566 9.8788 9.00086 9.8788Z" fill="currentColor"/>
            </svg>
        ...
        <input type="hidden" name="street_id" value="{{ old('street_id', $streetData?->id) }}" required>
        <input type="hidden" name="street_name" value="{{ old('street_name', $streetData?->name) }}">
        <input type="hidden" name="zone_id" value="{{ old('zone_id', $zoneData?->id) }}">
        <input type="hidden" name="zone_name" value="{{ old('zone_name', $zoneData?->name) }}">
        <input type="hidden" name="district_id" value="{{ old('district_id', $districtData?->id) }}">
        <input type="hidden" name="district_name" value="{{ old('district_name', $districtData?->name) }}">
        <input type="hidden" name="city_id" id="city_id" value="{{ old('city_id', $cityData?->id) }}">
        <input type="hidden" name="city_name" value="{{ old('city_name', $cityData?->name) }}">
    </div>
</div>
