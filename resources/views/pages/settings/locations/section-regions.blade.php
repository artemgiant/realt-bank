    ...
    <div class="card">
        <div class="card-body" style="padding:0;">
            @if(isset($states) && $states->total() > 0)
                <div class="tree-list" id="regionsTreeList">
                    @foreach($states as $state)
                        <div class="tree-item level-1" data-state-id="{{ $state->id }}" data-search="{{ mb_strtolower($state->name) }}">
                            @if($state->cities->flatMap->districts->count() > 0)
                                <span class="tree-expand">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14"><polyline points="9 18 15 12 9 6"/></svg>
                                </span>
                            @endif
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="16" height="16" style="color:var(--primary);flex-shrink:0"><polygon points="1 6 1 22 8 18 16 22 23 18 23 2 16 6 8 2 1 6"/><line x1="8" y1="2" x2="8" y2="18"/><line x1="16" y1="6" x2="16" y2="22"/></svg>
                            <span class="tree-name">{{ $state->name }}</span>
                            <span class="tree-count">{{ $state->cities_count }} {{ trans_choice('город|города|городов', $state->cities_count) }}</span>
                        ...
                        @foreach($state->cities as $city)
                            @foreach($city->districts as $district)
                                <div class="tree-item level-2" data-parent-state="{{ $state->id }}" data-search="{{ mb_strtolower($district->name) }}">
                                    <span class="tree-name">{{ $district->name }}</span>
                                    <span class="tree-city-label">{{ $city->name }}</span>
                                    <span class="tree-count">{{ $district->streets_count }} {{ trans_choice('улица|улицы|улиц', $district->streets_count) }}</span>
                                    <div class="actions-cell" style="margin-left:auto">
