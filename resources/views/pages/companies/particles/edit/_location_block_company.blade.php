{{-- ========== ЛОКАЦИЯ (Область + Улица) - EDIT MODE ========== --}}
<div class="item selects w16">
    ...
    <div class="state-search-wrapper">
        <input type="text"
               ...
               autocomplete="off"
               value="{{ old('state_name', $company->state?->name) }}"
               required>
        ...
        <input type="hidden" name="state_id" id="state_id" value="{{ old('state_id', $company->state_id) }}" required>
        <input type="hidden" name="state_name" id="state_name" value="{{ old('state_name', $company->state?->name) }}">
        <input type="hidden" name="country_id" id="country_id" value="{{ old('country_id', $company->country_id) }}">
        <input type="hidden" name="country_name" id="country_name" value="{{ old('country_name', $company->country?->name) }}">
    </div>
</div>
...
<div class="item w33">
    ...
    <div class="location-search-wrapper">
        <input type="text"
               ...
               autocomplete="off"
               value="{{ old('street_name', $company->street?->name) }}"
               required>
        ...
        <input type="hidden" name="street_id" value="{{ old('street_id', $company->street_id) }}" required>
        <input type="hidden" name="street_name" value="{{ old('street_name', $company->street?->name) }}">
        <input type="hidden" name="zone_id" value="{{ old('zone_id', $company->zone_id) }}">
        <input type="hidden" name="zone_name" value="{{ old('zone_name', $company->zone?->name) }}">
        <input type="hidden" name="district_id" value="{{ old('district_id', $company->district_id) }}">
...
<div class="item w16">
    <span>
        ...
        <label for="number-apartment">Офис</label>
    </span>
    <div class="item-inputText-wrapper shtrih">
        <input class="item-inputText" id="number-house" name="building_number" type="text" autocomplete="off" value="{{ old('building_number', $company->building_number) }}">
        <input class="item-inputText" id="number-apartment" name="office_number" type="text" autocomplete="off" value="{{ old('office_number', $company->office_number) }}">
    </div>
</div>
{{-- ========== /ЛОКАЦИЯ ========== --}}
