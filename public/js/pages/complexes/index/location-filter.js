(function () {
    ...
    const updatePlaceholder = () => {
        ...
        if (mode === 'detail') el.search.placeholder = 'Район, улица, ориентир...';
    ...
    const updateHidden = (triggerReload = true) => {
        ...
        // Обновляем таблицу комплексов (если не отключено)
        if (triggerReload && typeof window.reloadComplexesTable === 'function') {
            window.reloadComplexesTable();
        }
    };

    // Экспортируем метод reset для внешнего использования
