...
/**
 * Бизнес-логика обновления объекта недвижимости.
 *
 * Вся операция обёрнута в транзакцию:
 * обновление записи, переводы, контакты, особенности, документы, фотографии.
 */
class UpdateProperty
{
    /**
     * Обновить объект недвижимости со всеми связями.
     *
     * @param Property $property Обновляемый объект
     * @param array $data Валидированные данные из UpdatePropertyRequest
     * @param Request $request Исходный запрос (для файлов и features)
     * @return Property Обновлённый объект
     */
    public function execute(Property $property, array $data, Request $request): Property
    {
        return DB::transaction(function () use ($property, $data, $request) {
            $pricePerM2 = $this->calculatePricePerM2($data);

            $this->reassignAgent($property, $data);
            ...
            $this->syncContacts($property, $data);
            $this->syncFeatures($property, $request);
            $this->saveDocuments($property, $request);
            $this->updatePhotoOrder($property, $request);
    ...
    /** Переназначить агента, если передан assigned_agent_id */
    private function reassignAgent(Property $property, array $data): void
    {
    ...
    /** Сохранить/обновить переводы (заголовок и описание) для ua, ru, en */
    ...
    /** Синхронизировать контакты (клиентов) объекта */
    private function syncContacts(Property $property, array $data): void
    {
        $property->contacts()->sync($data['contact_ids'] ?? []);
    }

    /** Синхронизировать особенности объекта */
    private function syncFeatures(Property $property, Request $request): void
    {
        $property->features()->sync($request->input('features', []));
    }

    /** Сохранить новые документы в storage */
    ...
    /** Обновить порядок существующих фотографий */
    private function updatePhotoOrder(Property $property, Request $request): void
    {
    ...
    /** Загрузить новые фотографии через PhotoUploadService */
